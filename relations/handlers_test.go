...
const successfulContentCollectionResponse = `{"contains":[{"id":"http://id-f78c1482-a65c-413e-b753-ca3ce3cb84f0", "apiUrl":"http://apiurl-f78c1482-a65c-413e-b753-ca3ce3cb84f0"}],
"containedIn":[{"id":"http://id-f78c1482-a65c-413e-b753-ca3ce3cb84f0", "apiUrl":"http://apiurl-f78c1482-a65c-413e-b753-ca3ce3cb84f0"}]}`
...
func TestGetContentCollectionRelationsHandler(t *testing.T) {
	tests := []test{
		...
		{"NotFound", newRequest("GET", fmt.Sprintf("/contentcollection/%s/relations", "db90a9db-6cb6-4ba0-8648-c0676087aba2"), nil), &cypherDriverMock{contentUUID: knownUUID}, http.StatusNotFound, message("No relations found for contentcollection with uuid db90a9db-6cb6-4ba0-8648-c0676087aba2")},
	...
	for _, test := range tests {
		...
		rec := httptest.NewRecorder()
		r := mux.NewRouter()
		r.HandleFunc("/contentcollection/{uuid}/relations", hh.GetRelations).Methods("GET")
		r.ServeHTTP(rec, test.req)
		assert.True(t, test.statusCode == rec.Code, fmt.Sprintf("%s: Wrong response code, was %d, should be %d", test.name, rec.Code, test.statusCode))
		assert.JSONEq(t, test.body, rec.Body.String(), fmt.Sprintf("%s: Wrong body", test.name))
	}
...
func (cdm *cypherDriverMock) findContentCollectionRelations(contentUUID string) (relations, bool, error) {
	if cdm.failRead {
		return relations{}, false, errors.New("TEST failing to READ")
	}
	if contentUUID == cdm.contentUUID {
